...
(async function() {
    ...
    if(!sessionStorage.aVehicleTypesNew || JSON.parse(sessionStorage.aVehicleTypesNew).lastUpdate < (new Date().getTime() - 4 * 500 * 60)) {
            await $.getJSON("https://afiliafrostfang.github.io/afilia.github.io/vehicletype.json").done(data => sessionStorage.setItem('aVehicleTypesNew', JSON.stringify({lastUpdate: new Date().getTime(), value: data})) );
        }

    if(!sessionStorage.aMissions || JSON.parse(sessionStorage.aMissions).lastUpdate < (new Date().getTime() - 4 * 500 * 60)) {
        await $.getJSON('https://afiliafrostfang.github.io/afilia.github.io/einsaetze.json').done(data => sessionStorage.setItem('aMissions', JSON.stringify({lastUpdate: new Date().getTime(), value: data})) );
    }
...
overflow-y: auto;
                    ...
                    <div class="modal-dialog modal-lg" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          ...
                          <h5 class="modal-title"><center>Entweder du packst jetzt deine Einsätze und fährst!</center></h5>
                          <h5 class="modal-title"><center>Oder du packst deine Einsätze und gehst!</center></h5>
                          <div class="btn-group">
    ...
    $("#btn-group-mission-select")
        .before(`<a id="chilloutArea" data-toggle="modal" data-target="#chiAModal" class="btn btn-success btn-xs">
                   <span class="glyphicon glyphicon-queen"></span> Was'n hier los?!
                 </a>`);
        ...
        for(var i in allianceMissions) {
            ...
            for(var v = 0; v < checkboxes.length; v++) {
                ...
                if(v+1 == checkboxes.length) {
                    ...
                    $("#status_"+mId).text("Samma pennst du oder was? Wir haben keine Fahrzeuge!");
                    break;
                }
            }
        }
